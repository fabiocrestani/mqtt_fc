...
uint8_t mqtt_handle_received_connack(char *buffer, uint32_t len)
{
	...
	logger_log_mqtt(TYPE_INPUT, "CONNECT response received: CONNACK");
	...
	{
		logger_log_mqtt(TYPE_ERROR, "Error parsing CONNACK message");
		return FALSE;
	}
}
...
{
	logger_log_mqtt(TYPE_INPUT, "PUBLISH response received: PUBACK");
	...
	{
		logger_log_mqtt(TYPE_ERROR, "Error parsing PUBACK message");
		return FALSE;
	}
}
...
uint8_t mqtt_handle_received_pingresp(char *buffer, uint32_t len)
{
	...
	logger_log_mqtt(TYPE_INPUT, "PINGREQ response received: PINGRESP");
	...
	{
		logger_log_mqtt(TYPE_ERROR, "Error parsing PINGRESP message");
		return FALSE;
	}
}
...
uint8_t mqtt_handle_received_suback(char *buffer, uint32_t len)
{
	...
	logger_log_mqtt(TYPE_INPUT, "SUBSCRIBE response received: SUBACK");
...
uint8_t mqtt_handle_received_publish(char *buffer, uint32_t len)
{
	...
	logger_log_mqtt(TYPE_INPUT, "PUBLISH message received");
	...
	{
		logger_log_mqtt(TYPE_ERROR, "Error parsing PUBLISH message");
		return FALSE;
	}
}
...
void mqtt_queue_received_publish(Mqtt *mqtt, PublishMessage *message)
{
	if (mqtt->received_publish_counter >= MQTT_RECEIVED_PUBLISH_QUEUE_SIZE)
	{
		logger_log_mqtt(TYPE_ERROR,
			"Error! Queue of received publish messages is full!");
		return;
	}
